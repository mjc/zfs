...
run_as_root() {
  ...
  # fall back to sudo only if we have one available
  for cmd in /usr/bin/sudo /bin/sudo /run/wrappers/bin/sudo; do
    if [ -x "$cmd" ]; then
      "$cmd" "$@"
      return
    fi
  ...
  echo "error: no root helper available (doas/sudo)" >&2
  return 127
}
...
echo "Running Nix-friendly qemu host setup bootstrap."

if [ "${NIX_QEMU_SKIP_HOST_CLEANUP:-0}" = "1" ]; then
  echo "NIX_QEMU_SKIP_HOST_CLEANUP=1: skipping Ubuntu runner cleanup/package install steps."
else
...
if [ ! -x "$(command -v zpool || true)" ] || [ ! -x "$(command -v zfs || true)" ]; then
  echo "Host zfs tools missing in this environment; expected zpool/zfs from Nix package set."
  exit 1
fi
